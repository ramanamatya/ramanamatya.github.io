$(document).ready(function(){	
	...
	var hgrid = 2000;
	var lastClass;
	var projectsList = ["uiux", "birds", "spt", "cfsite", "mediaplayer", "ichigo"];
		...
		if (isOpening == true ) {
			...
			if (section == "about") {
				...
				for (var cc = 1; cc<=3; cc++) {
					if ( $(".job" + cc).isOnScreen() ) {
						$(".job" + cc).addClass("display");
					}
				}
			}
			if (section == "contact") {
	...
	function bgsize() {
		...
		if ($(window).width() < 960 ) { hgrid = $("#grid .content").height() + 400; }
        	...
        	if ($(window).width() > 1786) { hgrid = (409*3)*(1786/1440) + 600; }
        	else { hgrid = (409*3)*($(window).width()/1440) + 600; }
        }
	...
	function project(projectname) {
		setTimeout(function(){ 
			...
			openprojet();
			// $("#projet").load("projets", function(){ console.log("ok"); openprojet() }); 
		}, 1000);
	}
		...
		if (isOpening == false) {
			...
			for (var i = 0; i < projectsList.length; i++) { 
			    if (lastClass == projectsList[i]) {
			    	...
			    	if (i == 0) { 
			    		setTimeout(function(){  
				    		$("body").attr("class", "projet nav "+projectsList[5]);
				    		$(".previous").attr("class", "previous transition "+projectsList[5]);
				    		$(".next").attr("class", "next "+projectsList[5]);
				    		openprojet();
				    		// $("#projet").load("/projets/"+projectsList[5]+".html", function(){ console.log("ok"); openprojet() }); 
			    		}, 2000);
			    	} 
			    	else { 
			    		numprojet = i-1; 
	...
	$("#logosmall, .menuworks").click(function(){
		...
		$("#grid, #grid_bg").removeClass("enter");
	...
	$(".menublog").click(function(){
		window.open("http://ramanamatya.blogspot.com/");
	});
